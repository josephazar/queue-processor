...
class Function(BaseModel, ABC):
    ...
    def run(self, function_call: FunctionCall = None):
        # If no function_call was provided at all
        if function_call is None:
            # If you have no parameters, call the function with no arguments
            if not self.parameters:
                return self.function()
            else:
                # If you do have parameters and they're required, raise an error
                # or handle it however you see fit
                raise Exception("Missing function call and parameters are required")
